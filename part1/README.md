...
*The history command is used to track of the input the user has input.*

DESCRIPTION:

*The history command is used to list all of the user input with the most recent command listed at the bottom of the output. The command is a built-in command which can be called by the user in the mini-shell by typing history. The history record will be cleared when the user terminates or exits the mini-shell.*

NOTES:

*I used a custom data type queue (linkedlist implementation) to store all the input information through the createQueue and queueEnqueue functions. The maximum size of each command string is sizeof(char) times 80. The maximum number of history commands can be stored in the queue is also 80 (stored as a BUFFER_SIZE constant). When the historyQueue has a maximum of 80 commands, the eldest command line will be dequeued from the historyQueue and the newest input command line will be added to the tail/back of the queue. Therefore, the user will be able to see the most recent 80 items of input history during each round of mini-shell operations. I used a loop to output the record of the history queue using printHistory() function.*
