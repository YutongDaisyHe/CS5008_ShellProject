...
int main(){
  alarm(180); 
  queue_t* historyQ=createQueue(); // Initialize history queue
  char line[BUFFER_SIZE]; // Initialize input string array

  // Initialize an array to store all available commands
  // Index 4, 5, 6 indicates commands with special function call
  ...
  while(1){
      // Initialize inputCommand and reset input line for storing new input
      char** inputCommand=createArray(); // Initialize inputCommand
      reset(line);
      readLine(line, historyQ); // Take user input 
      // The returned value indicates valid commands or -1 if invalid
      int task=isCommand(line, commandArray);
      // Built-in command is called
      int commandCount = parseLine(line, inputCommand); 
      if(task >= 0 && task < 4){ // Call the requested built-in command
          commandPfnArray[task](inputCommand, commandArray);
      }
      else if(4==task){ // history is called
          printHistory(historyQ);
      }
      else if(5==task){ // exit is called
          freeArray(inputCommand);
          break;
      }
      else if(6==task){ // Pipe command is called
          // Initialize arrays to store commands at the sides of the pipe sign
